class PurchaseRecordsController < ApplicationController
  ...
  def purchase_records_destination_params
    params.require(:purchase_records_destination).permit( :post_code, :prefecture_id, :city, :phon_number, :building, :address).merge(user_id: current_user.id, item_id: params[:item_id], token: params[:token])  
  end
